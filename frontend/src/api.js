...
import './App.css';
import { signup, login, predict,
         getHistory, deleteHistory } from './api';
...
function App() {
  ...
  const [preview, setPreview]   = useState(
    () => localStorage.getItem('pn-preview') || null
  );
  const [result, setResult]     = useState(() => {
    try {
      const s = localStorage.getItem('pn-result');
  ...
  const [patientName, setPatientName] = useState(
    () => localStorage.getItem('pn-name') || ''
  );
  const [patientAge, setPatientAge] = useState(
    () => localStorage.getItem('pn-age') || ''
  );
  ...
  useEffect(() => {
    localStorage.setItem('pn-name', patientName);
  }, [patientName]);

  useEffect(() => {
    localStorage.setItem('pn-age', patientAge);
  }, [patientAge]);

  useEffect(() => {
    if (result) localStorage.setItem('pn-result', JSON.stringify(result));
    else localStorage.removeItem('pn-result');
  }, [result]);

  useEffect(() => {
    if (preview) localStorage.setItem('pn-preview', preview);
    else localStorage.removeItem('pn-preview');
  }, [preview]);
  ...
  const handleAuth = async () => {
    ...
    try {
      ...
      if (authPage === 'signup') {
        ...
        data = await signup(authName, authEmail, authPass);
      } else {
        data = await login(authEmail, authPass);
      }
      ...
      setAuthError('Server error! Backend chalu hai?');
    } finally {
      setAuthLoad(false);
    }
  };
  ...
  const handleLogout = () => {
    ...
    localStorage.removeItem('pn-result');
    localStorage.removeItem('pn-preview');
    setPage('home');
  };
  ...
  const handleFileChange = (file) => {
    ...
    if (file.type !== 'image/jpeg' && file.type !== 'image/png') {
      setError('Only JPEG and PNG!'); return;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const data = await predict(selectedFile, token);
      if (data.status === 'success') {
        const record = {
          id:          Date.now(),
  ...
  const handleReset = () => {
    ...
    localStorage.removeItem('pn-result');
    localStorage.removeItem('pn-preview');
    localStorage.removeItem('pn-name');
    localStorage.removeItem('pn-age');
  };
  ...
  return (
    <div className="app">
        ...
        <section className="hero">
          ...
          <div className="hero-inner">
            ...
            <p className="hero-desc">
              Welcome back, <strong>{user?.name}</strong>! Upload a Chest X-Ray to analyze.
            </p>
            ...
            {(selectedFile || preview) && (
              <div className="ftag">
                ...
                <span className="ftag-name">
                  {selectedFile ? selectedFile.name : 'Previously uploaded'}
                </span>
                {selectedFile &&
              ...
              <div className={`rcard ${result.isPneumonia?'rc-danger':'rc-success'} fadein`}>
                ...
                {result.gradcam && (
                  <div className="gradcam-wrap">
                    ...
                    <p className="gradcam-hint">
                      🔴 Red = AI ne yahan zyada focus kiya
                    </p>
                  </div>
                )}
                ...
                <div className="advice-block">
                  ...
                  {(result.isPneumonia ? [
                    ...
                    '📞 Call emergency if breathing difficult',
                  ]:[
        ...
        <main className="hist-page">
          ...
          {histLoad ? (
            <div className="loading-state">
              ...
              <p className="load-t">Loading history...</p>
            </div>
          ) : dbHistory.length === 0 ? (
      ...
      <footer className="footer">
        <span>🫁 PneumoAI — PyTorch + FastAPI + React</span>
        <span className="foot-dot">·</span>
        <span>⚠️ Educational purposes only. Consult a doctor.</span>
      </footer>
    </div>
  );
}
